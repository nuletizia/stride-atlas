...
// Internal keys stay as `tempo`/`intervals` so CSS variables (--type-tempo,
// --type-intervals), theme palettes, and component arrays don't need to
// shift. Display labels convey *intensity* (Moderate / Hard) — from a
// Strava summary we can only tell how hard a run was, not whether it was
// structured as intervals vs tempo. Phase 2 (lap fetching) will let us
// re-introduce structural labels under the intensity buckets.
export const TYPE_META = {
  easy:      { label: 'Easy',     targetPace: 5.45, drift: 0.35, hrAvg: 142 },
  tempo:     { label: 'Moderate', targetPace: 4.35, drift: 0.15, hrAvg: 168 },
  long:      { label: 'Long',     targetPace: 5.15, drift: 0.20, hrAvg: 150 },
  intervals: { label: 'Hard',     targetPace: 3.55, drift: 0.25, hrAvg: 176 },
...
// Estimate the athlete's HRmax from their highest observed single-run max HR.
// Add a 2% buffer (true max is always a bit above recorded peak unless they
// hit a track-test spike). Floor at 190 (a safe default for most adults) when
// data is sparse or missing. Cap at 220.
export function estimateHrMax(getMaxHr) {
  let observed = 0;
  for (const mh of getMaxHr) {
    const v = Number(mh || 0);
    if (isFinite(v) && v > observed) observed = v;
  }
  if (observed < 160) return 190;
  return Math.min(Math.round(observed * 1.02), 220);
}
...
// Unified workout-type inference. Caller normalises whatever signals it
// has (CSV columns, Strava API enums, HR streams) into the inputs here.
//
// Priority:
//   1. Explicit flags (isRace / isLong / isRecovery, name regex)
//   2. Structural auto-detect (distance > 15 km → long; short + low HR → recovery)
//   3. Laps-based intervals detection (offline mode, when laps are available)
//   4. Intensity bands from HR% and suffer_score (summary mode)
//
// Internal return values are still the 6 original keys (easy/tempo/long/
// intervals/race/recovery) so CSS / theme / components don't change. What
// the user SEES is controlled by TYPE_META labels: `tempo` = "Moderate",
// `intervals` = "Hard". Rationale: from a Strava summary alone we can't
// tell structured intervals apart from sustained tempo — only intensity.
export function inferType({
  name = '', description = '',
  isRace = false, isLong = false, isRecovery = false,
  distKm, durMin, avgHr,
  maxHr = null, sufferScore = null,
  laps = null, hrMax = 190,
}) {
  // ---- 1. Explicit flags ----
  ...
  if (distKm < 5.5 && avgHr && avgHr / hrMax < 0.65) return 'recovery';

  // ---- 3. Laps-based intervals (offline mode: FIT streams include laps) ----
  if (laps?.length >= 5) {
  ...
  // ---- 4. Intensity bands (summary mode: no laps, only HR + suffer) ----
  // We combine avg HR and max HR into a single intensity score (the mean
  // of the two, as % of HRmax). This avoids two failure modes:
  //   - avg-only → tempo intervals score low because rest pulls avg down
  //   - max-only → any hill surge on an easy run crosses the threshold
  //
  // suffer_score is intentionally NOT used as a classification threshold:
  // it's computed against the athlete's own HR zones, which vary by person,
  // so fixed thresholds over- or under-fire unpredictably. It's still
  // surfaced elsewhere for display, just not here.
  //
  // Calibration note: these thresholds assume the configured hrMax matches
  // the athlete's actual max HR. If you see too many runs flagged Hard,
  // your hrMax is probably set too low. Bump it via HR_MAX env var at
  // ingest time or in Strava's zone settings.
  const hrPct = (avgHr || 0) / hrMax;
  const maxHrPct = maxHr ? maxHr / hrMax : hrPct;
  const intensityScore = (hrPct + maxHrPct) / 2;

  if (intensityScore > 0.91 && distKm >= 3) return 'intervals';
...
//   id, name, distance (m), moving_time (s), elapsed_time (s),
...
export function summaryToRun(activity, { hrMax = 190 } = {}) {
  ...
  // Stopped-time share = 1 - moving/elapsed. Both fields ship in the summary
  // payload, so no extra API call. Drives isInterrupted() in the UI.
  ...
  const maxHrRaw = Number(activity.max_heartrate);
  ...
  // Strava reports average_temp in Celsius, but only when the recording
  // device captured it (most GPS watches do; phones usually don't). Absent
  // → null, and every consumer hides the field rather than showing a zero.
  const tempRaw = Number(activity.average_temp);
  const temp = isFinite(tempRaw) ? Math.round(tempRaw) : null;

  // Strava calls it suffer_score in the API; in the UI it's "Relative Effort".
  const sufferRaw = Number(activity.suffer_score);
  ...
  return {
    ...
    startLat, startLng,
    note: activity.name || '',
    // Raw classification inputs so the dashboard can re-run inferType when
    // the user overrides HRmax. Description is empty in summary mode.
    isRace: activity.workout_type === 1,
    isLong: activity.workout_type === 2,
    isRecovery: false,
    sufferScore,
...
export function buildStrideData(activities, athlete = null, { hrMax = 190 } = {}) {
  ...
  const runs = activities.map((a) => ({
    ...
    hr: a.hr, maxHr: a.maxHr ?? null,
    elev: a.elev, temp: a.temp ?? null, zones: a.zones,
    bestSplits: a.bestSplits || null,
    pr: a.pr, routePR: a.routePR, note: a.note,
    // Classification inputs — let the client re-run inferType when the user
    // overrides HRmax. Offline FIT runs don't populate isRace/isLong/
    // isRecovery, so reclassification for those relies on distance + HR.
    isRace: a.isRace || false,
    isLong: a.isLong || false,
    isRecovery: a.isRecovery || false,
  ...
  // Highest per-run max HR ever observed. Used to display the athlete's
  // recorded peak alongside the (estimated or overridden) HRmax we use
  // for zones and intensity classification.
  let hrMaxObserved = 0;
  for (const a of activities) {
    const v = Number(a.maxHr || 0);
    if (isFinite(v) && v > hrMaxObserved) hrMaxObserved = v;
  }
  ...
  return {
    ...
    streamsSynced,
    hrMax,
    hrMaxObserved: hrMaxObserved || null,
    zonePct: ZONE_PCT,
  };
}
